...
def load_yolo_model():
    ...
    try:
        ...
        if os.path.exists('best.pt'):
            ...
            logger.warning("⚠️ YOLO model file 'best.pt' not found. Image categorization will be limited.")
            return False
    except ImportError:
        logger.warning("⚠️ ultralytics not installed. Install it with: pip install ultralytics.")
        return False
...
def categorize_image_with_yolo(image_path):
    try:
        ...
        logger.info(f"🖼️ Running YOLO prediction on: {image_path}")

        results = yolo_model.predict(image_path, verbose=False)

        if not results or len(results) == 0:
            logger.error("⚠️ No results from YOLO prediction")
            return None
        ...
        if hasattr(result, 'probs') and result.probs is not None:
            ...
            if not prob_data:
                logger.error("⚠️ No probability data from YOLO")
                return None
        ...
        # Object detection case
            ...
            logger.error("⚠️ Unexpected YOLO result format")
